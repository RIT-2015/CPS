...
def padIntTo32Bits(data):
    ...
    """
    binString = bin(data)
    length = 32 - len(binString[2:])
    return binString[:2] + "0" * length + binString[2:]
...
def rightShift(data, amount):
    ...
    """
    amount = 34 - amount
    return data[:2] + data[amount:] + data[2:amount]
...
def Sum0(A):
    ...
    for index in range(2, 34):
        str = A2[index] + A13[index] + A22[index]
        if str.count('1') % 2 == 1:
            result += '1'
        else:
            result += '0'
...
def Sum1(E):
    ...
    result = "0b"
    for index in range(2, 34):
        str = E6[index] + E11[index] + E25[index]
        if str.count('1') % 2 == 1:
            result += '1'
        else:
            result += '0'
